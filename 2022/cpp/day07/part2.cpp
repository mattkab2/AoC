...
class Dir {
    ...
    int find_closest(int target, int curr) {
        if (this->isFile) {return curr;}
        for (auto const& [key, val] : this->members) {
            curr = val->find_closest(target,curr);
            if (val->size > target and val->size < curr and not val->isFile) {curr = val->size;}// cout << val->name << " " << val->size << '\n';}
        }
        return curr;
    }
};
...
int main(int argc, char *argv[]) {
    ...
    root->print();
    root->compute_sizes();
    int free_space, target;
    free_space = 70000000 - root->size;
    target = 30000000 - free_space;

    int ret = root->find_closest(target,root->size);
