    ...
    for (const auto &[dist, pair] : nearest) {
        ...
        if (boxToCircuit.contains(pair.first) && boxToCircuit.contains(pair.second) && boxToCircuit.at(pair.first) == boxToCircuit.at(pair.second)) {
            ...
            if (boxToCircuit.contains(pair.first) && boxToCircuit.contains(pair.second)) {
                ...
                circuits.erase(origCircuit);
                // Check if this was the final connection
                std::cout << "  Merged circuit has size " << circuits[circIndex].size() << std::endl;
                if (circuits[circIndex].size() == 1000) {
                    ret = coords[pair.first].at(0) * coords[pair.second].at(0);
                }
                circIndex++;
        ...
        if (circuits.size() == 1 && circuits[circuits.begin()->first].size() == 1000) {
            ret = coords[pair.first].at(0)*coords[pair.second].at(0);
            break;
        }
    }
