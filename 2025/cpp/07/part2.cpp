...
int main(int argc, char *argv[]) {
    ...
    std::vector<std::vector<long>> state;
    ...
    for (size_t row = 0; row < geometry.size(); row++) {
        for (size_t col = 0; col < geometry.front().size(); col++) {
            ...
            if (row > 0 && state[row-1][col] > 0) {
                ...
                } else if (geometry[row][col] == 1 && state[row-1][col] > 0) {
                    numSplits++;
                    if (geometry[row][col-1] == 0) {state[row][col-1] += state[row-1][col];}
                    if (geometry[row][col+1] == 0) {state[row][col+1] += state[row-1][col];}
                }
            }
        }
